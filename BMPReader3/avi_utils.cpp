...
// Desc:
//-----------------------------------------------------------------------------
HAVI CreateAvi( const char * fn, int frameperiod )
{
    IAVIFile * pfile;
    // The AVIFileInit function initializes the AVIFile library.
    AVIFileInit();
...
HRESULT CloseAvi( HAVI avi )
{
    ...
    // audio stream, initialised when audio stream is first created
    if ( au->as != 0 )
...
HRESULT SetAviVideoCompression( HAVI avi, HBITMAP hbm, AVICOMPRESSOPTIONS * opts, bool ShowDialog, HWND hparent )
{
    ...
    // if true, then no function will do anything (какая-то из функций установила ошибку)
    if ( au->iserr )
        return AVIERR_ERROR;

    // video stream, when first created ( уже присутствует поток )
    if ( au->psCompressed != 0 )
    ...
    if ( au->psCompressed == 0 )
        {
            ...
            ZeroMemory( &myopts, sizeof(AVICOMPRESSOPTIONS) );
    ...
    if ( au->psCompressed == 0 )
        {
            ...
            // The AVIMakeCompressedStream function creates a compressed stream from an uncompressed stream and a compression filter, and returns the address of a pointer to the compressed stream.
